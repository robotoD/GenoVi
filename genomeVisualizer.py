...
import glob
#import cairo # The import is actually lower in the script and it is not always strictly necessary, so this should stay commented.
...
def visualizeGenome(gbk_file, output = "circos", 
    ...
    if separate:
        ...
        for contig in contigs:
            ...
            createConf.create_conf(maxmins, GC_content, GC_skew, CDS_positive, CDS_negative, cogs, cogs_p, cogs_n)
        ...
        maxmins = GC_analysis.makeGC("temp/gbk_converted.fna", "temp/GC")
        createConf.create_conf(maxmins, GC_content, GC_skew, CDS_positive, CDS_negative, cogs, cogs_p=cogs_p, cogs_n=cogs_n)
...
def get_args():
    ...
    parser.add_argument("-c", "--cogs_classified", action='store_false', help="Do not clasify each coding sequence and draw them by color.", required = False)
    ...
    args.cogs_classified, args.legend_not_included, args.separate_circles, args.circles_alignment,
